...
<%
        ...
        if (request.getParameter("target").equals("recipe")) {
            if (request.getParameter("action").equals("delete")) {
                try {
                    if (Admin.deleteRecipe(id)) {
                        response.sendRedirect("admin_recipes.jsp?status=1");
                    } else {
                        response.sendRedirect("admin_recipes.jsp?status=2");
                    }
        ...
        <div id="wrapper">
            ...
            <div class="d-flex flex-column" id="content-wrapper">
                <div id="content">
                    ...
                    <div class="container-fluid">
                        <h3 class="text-dark mb-4">Recipe Details</h3>
                                    ...
                                    <div class="table-responsive table mt-2" id="dataTable" role="grid" aria-describedby="dataTable_info">
                                        <table class="table my-0" id="dataTable">
                                            ...
                                            <tbody>
                                                <%
                                                    try {
                                                        ResultSet rs = Admin.getRecipeDetails();
                                                        int i = 0;
                                                        while (rs.next()) {
                                                            i++;
                                                %>
                                                <tr>
                                                    ...
                                                    <td><%=rs.getString("Serving_Size")%></td>
                                                    <td><%=rs.getString("Prep_Time")%> min</td>
                                                    <td><%=rs.getString("Ingredients")%></td>
                                                    <td><%=rs.getString("Instructions")%></td>
                                                    <td>
                                                        <img src="assets/img/<%=rs.getString("Image")%>" width="100" height="100">
                                                    </td>
                                                    <td>
                                                        <a type="button" class="btn btn-danger" href="admin_recipes.jsp?target=recipe&action=delete&id=<%=rs.getString("Recipe_Id")%>" name="delete">Delete</a>
                                                    </td>
                                                </tr>
                                                <%
                                                    }
